...
# This function reads the distance matrix from the file user uploaded
...
def upgma(matrix, length, dictionary):
    ...
    while (length > 1):
        ...
        size = 0
        if leaves[min_index_i] not in list(dictionary.keys()):
            size = 1
            distance1 = distance
        else:
        ...
        if leaves[min_index_j] not in list(dictionary.keys()):
            size = size + 1
            distance2 = distance
        else:
...
def printCluster(dictionary, finalCluster):
    ...
    while stack:
        ...
        elif current in list(dictionary.keys()):
